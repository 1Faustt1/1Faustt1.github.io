...
const tasksData = [] // пустой массив в который потом добавляются задачи согласно предмету
...
const tasksCount = tasksData.length // информация по количеству заданий
...
for (let i = 0; i < tasksCount; i++) { // пока все задания не обработаются цикл работает
    ...
    card.childNodes[1].innerHTML = tasksData[i].name // вписывается заголовок
    card.childNodes[3].innerHTML = tasksData[i].description // вписывается описание (само задание)
    card.childNodes[7].childNodes[1].childNodes[1].innerHTML = "Номер: " + tasksData[i].id // вписывается идентификационный номер задания
    tasksContainer.appendChild(card) // готовая карточка добавляется в блок
    // console.log(card.childNodes)
}

tasksContainer.removeChild(templateCard) // со страницы убирается шаблок карточки
